...
const Navbar = () => {
  ...
  const navRef = useRef(); // For mobile menu
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (navRef.current && !navRef.current.contains(event.target)) {
        setIsOpen(false); // Close mobile menu when clicking outside
      }
    };
  ...
  return (
    <header className={`fixed w-full top-0 left-0 bg-transparent z-50 transition-transform ${isVisible ? "translate-y-0" : "-translate-y-full"}`}>
      <nav ref={navRef} className="container mx-auto flex justify-between items-center px-4 py-2 relative">
        ...
        <div
          className={`fixed top-0 right-0 h-screen w-full bg-gray-900 shadow-lg p-5 transition-transform transform ${
            isOpen ? "translate-x-0" : "translate-x-full"
          } lg:relative lg:translate-x-0 lg:bg-transparent lg:shadow-none lg:h-auto lg:w-auto lg:p-0 lg:flex`}
        >
          <ul className="flex flex-col space-y-4 lg:flex-row lg:space-y-0 lg:space-x-6">
            {["Home", "Board Games", "Retro Games", "Puzzles", "Consoles"].map((item) => (
              <li key={item}>
                <NavLink
              ...
              {userInfo ? (
                <>
                  ...
                  {isDropdownOpen && (
                    <ul
                      ref={dropdownRef}
                      className="absolute left-0 mt-2 w-48 bg-gray-900 border border-indigo-500 rounded-lg shadow-lg text-indigo-400 z-50 flex flex-col"
                    >
                      <li>
                        <NavLink
                          to="/order-history"
