...
import { userLogoutAction } from "../redux/Actions/User"; // Adjust the path as needed
  ...
  return (
    <header className={`fixed w-full top-0 left-0 bg-transparent z-50 transition-transform duration-300 ${isVisible ? 'transform translate-y-0' : 'transform -translate-y-full'}`}>
      <nav className="container mx-auto flex justify-between items-center px-4 py-2">
        ...
        <div className={`fixed top-0 right-0 h-screen w-full bg-gray-900 shadow-lg p-5 transition-transform transform ${isOpen ? 'translate-x-0' : 'translate-x-full'} overflow-y-auto lg:relative lg:translate-x-0 lg:bg-transparent lg:shadow-none lg:h-auto lg:w-auto lg:p-0 lg:flex`}>
          <ul className="flex flex-col space-y-4 lg:flex-row lg:space-y-0 lg:space-x-6">
            ...
            <li>
              {userInfo ? (
                <UserDropdown logoutHandler={handleLogout} />
              ) : (
